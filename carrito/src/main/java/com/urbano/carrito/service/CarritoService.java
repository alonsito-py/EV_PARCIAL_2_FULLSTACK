...
public class CarritoService {
    ...
    private void verificarStock(String sku, Integer cantidad) {
    ...
    public Carrito getOrCreateCarrito(Long clienteId) {
    ...
    public Carrito agregarItem(Long clienteId, ItemCarritoDTO dto) {
        ...
        verificarStock(dto.getSku(), dto.getCantidad());
        Carrito c=getOrCreateCarrito(clienteId);
    ...
    public void vaciarCarrito(Long carritoId) { log.info("Vaciando carrito id: {}", carritoId); Carrito c=repo.findById(carritoId).orElseThrow(()->new CarritoNotFoundException("Carrito no encontrado")); c.getItems().clear(); repo.save(c); }
    public void procesarCarrito(Long carritoId) { Carrito c=repo.findById(carritoId).orElseThrow(()->new CarritoNotFoundException("Carrito no encontrado")); c.setEstado("PROCESADO"); repo.save(c); }
}
