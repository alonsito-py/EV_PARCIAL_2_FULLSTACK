...
public class BodegaService {
    ...
    public MovimientoInventario registrar(MovimientoDTO dto) {
        ...
        m.setSku(dto.getSku()); m.setTipo(dto.getTipo()); m.setCantidad(dto.getCantidad()); m.setMotivo(dto.getMotivo());
        MovimientoInventario saved = repo.save(m);
        String endpoint = dto.getTipo().equals("ENTRADA")
            ? "/api/v1/inventario/sku/" + dto.getSku() + "/aumentar?cantidad=" + dto.getCantidad()
            : "/api/v1/inventario/sku/" + dto.getSku() + "/reducir?cantidad=" + dto.getCantidad();
        try {
            webClientInventario.patch().uri(endpoint).retrieve().bodyToMono(Object.class).block();
            log.info("Inventario actualizado sku: {}", dto.getSku());
